...
function exportParams() {
	...
	floatIP1=`grep 'floatIP1' ${PARAMS_FILE} | awk -F'|' '{print $2}' | sed -e 's/^ *//g;s/ *$//g'`
	floatIP2=`grep 'floatIP2' ${PARAMS_FILE} | awk -F'|' '{print $2}' | sed -e 's/^ *//g;s/ *$//g'`
	floatIP3=`grep 'floatIP3' ${PARAMS_FILE} | awk -F'|' '{print $2}' | sed -e 's/^ *//g;s/ *$//g'`
	subnetID=`grep 'subnetID1' ${PARAMS_FILE} | awk -F'|' '{print $2}' | sed -e 's/^ *//g;s/ *$//g'`
	subnetID2=`grep 'subnetID2' ${PARAMS_FILE} | awk -F'|' '{print $2}' | sed -e 's/^ *//g;s/ *$//g'`
	subnetID3=`grep 'subnetID3' ${PARAMS_FILE} | awk -F'|' '{print $2}' | sed -e 's/^ *//g;s/ *$//g'`
	routeTableID=`grep 'routeTableID' ${PARAMS_FILE} | awk -F'|' '{print $2}' | sed -e 's/^ *//g;s/ *$//g'`
...
curl http://localhost/occm/api/occm/setup/init -v -X POST --header 'Content-Type:application/json' --header 'Referer:AWSQS2' --retry 20 --connect-timeout 60 --data '{ "proxyUrl": { "uri": "" }, "userRequest":{  "email": "'${adminEmail}'","lastName": "admin", "firstName":"admin","accessKey": "nokeys","roleId": "Role-1","secretKey": "nokeys","password": "'${adminPassword}'"  }, "site": "'${customer}_site'", "company": "'${customer}'","tenantRequest": { "name": "'${customer}_tenant'", "description": "", "costCenter": "", "nssKeys": {} }}' >> /tmp/occm-init.txt
...
curl http://localhost/occm/api/auth/login -v --retry 20 --connect-timeout 60 --header 'Content-Type:application/json' --header 'Referer:AWSQS2' --data '{"email":"'${adminEmail}'","password":"'${adminPassword}'"}' --cookie-jar cookies.txt
sleep 5
...
name=$(tr '-' 'x' <<< ${name:0:30})
curl http://localhost/occm/api/aws/ha/working-environments  -X POST --cookie cookies.txt --header 'Content-Type:application/json' --header 'Referer:AWSQS2' --data '{"name":"'${name}otcHAmulti'","tenantId":"'${tenantId}'","region":"'${region}'","ebsVolumeType":"gp2","ebsVolumeSize": {"size": 1, "unit": "TB", "_identifier": "1 TB"},"haParams": {"clusterFloatingIP":"'${floatIP1}'","dataFloatingIP":"'${floatIP2}'","dataFloatingIP2":"'${floatIP3}'","failoverMode":"FloatingIP","node1SubnetId":"'${subnetID}'","node2SubnetId":"'${subnetID2}'","mediatorSubnetId":"'${subnetID3}'","routeTableIds": [
      "'${routeTableID}'"
    ],"mediatorKeyPairName":"'${keyPair}'"},"dataEncryptionType":"AWS","ontapEncryptionParameters":null,"skipSnapshots": "true","svmPassword":"'${svmPassword}'","vpcId":"'${vpcID}'","vsaMetadata":{"platformLicense":null,"ontapVersion":"latest","useLatestVersion": true,"licenseType":"ha-cot-standard-paygo","instanceType":"c4.2xlarge"}}' > /tmp/createVSA.txt
